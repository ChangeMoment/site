...
const copy = {
  ...
  fa: {
    bita: {
      ...
      credentials: "MA, RCC · BCACC #19727",
      bioParagraphs: [
        "ChangeMoment را از یک باور ساده بنا کردم: تغییر معنادار می‌تواند از یک لحظه‌ی کوچک آغاز شود — لحظه‌ای که خودمان را متفاوت می‌بینیم، الگویی را می‌شناسیم، یا امکانی تازه می‌یابیم. به نظر من، تغییر یک مقصد نیست؛ سفری پیوسته است برای آگاه‌تر شدن، ارتباط عمیق‌تر با خود، و هم‌راستا شدن با زندگی‌ای که می‌خواهیم.",
        "دارنده‌ی مدرک کارشناسی ارشد روانشناسی و دو مدرک کارشناسی در مشاوره‌ی بالینی و جامعه‌شناسی هستم. این پیشینه به من کمک کرده تا انسان‌ها را نه‌تنها از منظر تجربیات فردی‌شان، بلکه از طریق روابط، فرهنگ‌ها، جوامع و محیط‌های اجتماعی که شکل‌شان داده‌اند، بفهمم. بیش از یک دهه است که با بزرگسالان و زوج‌ها در دامنه‌ای گسترده از چالش‌های هیجانی، رابطه‌ای و زندگی کار می‌کنم.",
        "باور دارم مشاوره چیزی فراتر از یادگیری مهارت‌های کنارآمدن یا یافتن راه‌های کوتاه‌مدت برای تغییر رفتار است. ابزارهای عملی می‌توانند مفید باشند، اما تغییری عمیق‌تر و ماندگارتر اغلب از فهمیدن آنچه زیر رنج ماست سرچشمه می‌گیرد — احساسات، ترس‌ها، الگوهای رابطه‌ای، و تجربیاتی که نگاه ما به خودمان و دیگران را شکل داده‌اند. معتقدم هر یک از ما ظرفیتی برای بهبود و رشد در خود داریم و درمان می‌تواند از طریق همدلی، شفقت، کنجکاوی و ارتباط انسانی واقعی، ما را دوباره به این ظرفیت متصل کند.",
        "تجربه‌ی شخصی من از مهاجرت نیز به شکل عمیقی بر شیوه‌ی کارم تأثیر گذاشته است. زندگی میان دو فرهنگ به من درکی شخصی از تغییراتی داده که مهاجرت می‌تواند در هویت، حس تعلق، روابط و ارتباط ما با خودمان ایجاد کند. این تجربه به من اجازه می‌دهد مراجعان مهاجر را با هم دانش حرفه‌ای و هم درک تجربی ملاقات کنم، و در عین حال احترام بگذارم که هر داستان مهاجرتی منحصربه‌فرد است.",
        "فراتر از کار بالینی‌ام، به عنوان رئیس هیئت‌مدیره‌ی TrueNetwork Community Solutions Society، یک سازمان غیرانتفاعی در بریتیش‌کلمبیا، فعالیت می‌کنم. این مشارکت اجتماعی بازتاب باور گسترده‌تر من است که سلامت ما نه‌تنها توسط آنچه درون‌مان می‌گذرد، بلکه توسط خانواده‌ها، فرهنگ‌ها، جوامع و محیط‌هایی که بخشی از آن‌ها هستیم، شکل می‌گیرد.",
      ],
      quoteText: "درمان زمانی قدرتمندترین است که فضایی برای درک واقعی ایجاد کند — از خودمان، از الگوهایمان، و از روابطی که شکل‌مان داده‌اند.",
      approachEyebrow: "چارچوب درمانی",
      approachHeading: "رویکرد من به درمان",
      approachItems: [
        { title: "روان‌پویایی روان‌درمانی", body: "ریشه‌های عمیق‌تر هیجانات و الگوهای تکرارشونده را با هم بررسی می‌کنیم تا خودشناسی بیشتری بیابیم و فضایی برای تغییری معنادار و پایدار ایجاد کنیم." },
        { title: "درمان هیجان‌مدار (EFT)", body: "با هیجانات زیرین و الگوهای رابطه‌ای کار می‌کنیم تا از بهبود، ارتباط عمیق‌تر و شیوه‌های امن‌تر ارتباطی حمایت کنیم." },
        { title: "رویکرد مبتنی بر دلبستگی", body: "بررسی می‌کنیم چگونه روابط مهم زندگی، نحوه‌ی تجربه‌ی ما از اعتماد، نزدیکی و امنیت را شکل داده‌اند و به شما کمک می‌کنیم پیوندهایی سالم‌تر و امن‌تر بسازید." },
        { title: "رویکرد مبتنی بر ذهن‌آگاهی", body: "آگاهی از افکار و هیجانات را با قضاوت کمتر می‌پرورانیم و فضایی برای پذیرش، رشد و پاسخ‌های آگاهانه‌تر ایجاد می‌کنیم." },
      ],
      focusEyebrow: "حوزه‌های تخصصی",
      focusHeading: "با افتخار کنار چه کسانی کار می‌کنم",
      focusItems: [
        { title: "اضطراب، افسردگی، فقدان و سوگ", body: "یافتن آرامش بیشتر، تعادل پایدارتر و حس تازه‌ای از امید." },
        { title: "تروما رابطه‌ای و دلبستگی", body: "بازیابی امنیت، اعتماد و ظرفیت برای ارتباطی سالم‌تر." },
        { title: "زوج‌ها و بحران‌های رابطه‌ای", body: "تقویت درک متقابل، گفت‌وگو و صمیمیت." },
        { title: "هویت و ارزش خود", body: "پرورش خودشناسی عمیق‌تر، اعتمادبه‌نفس و پذیرش خود." },
    ...
    steps: ["می‌گویید چه چیزی شما را به اینجا رسانده", "با هم به آنچه می‌گذرد معنا می‌دهیم", "قدم بعدی را با آرامش برمی‌دارید"],
  },
  fr: {
    bita: {
...
export function Team() {
  ...
  return (
    <>
      ...
      <CTABand heading={page.ctaTitle} body={page.ctaBody} />
    </>
  );
}
